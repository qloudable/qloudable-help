# Create Demo Lab Help Text

You can read the instructions and tips here to help you create your demo lab. At any time in the process, you can click **Save As Draft** in the bottom-left corner of the window to save your progress and come back to it later. 

For further assistance, send an email to support@qloudable.com.

## Basic Details

On this page, you will provide the basic information about your demo lab.

### Name of the Demo Lab

In this field, give your demo lab a unique name. This will be the name that customers see when they are browsing demo labs, so make sure it is a name that aligns with your product name appropriately, stands out, and demonstrates what your demo lab does.

![demo lab name gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-name.gif)

### Name of the Publisher

Put your company or organization's name here so customers know who made the demo lab they're about to try.

![publisher name gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-publisher-name.gif)

### Demo Lab Description

Here you can put a quick description of what you demo lab is and what it does. This will help customers decide if they want to try your demo lab, so be descriptive.

![lab description gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-description.gif)

### Contact Email

Enter the email that will be provided to customers who have questions or concerns. Make sure this email is actively monitored by support staff.

![contact email gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-contact-email.gif)

### Error Email

Enter the email address where you wish to receive error logs and reports for this demo lab.

![error email gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-error-email.gif)

### Demo Lab Icons

Here you can upload up to four images that will serve as the icons for this demo lab. You can also hover over icons you have already uploaded to edit or remove them.

![demo lab icons gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-icons.gif)

### Preview Images

Here you can upload images that will be placed into a slideshow on the demo lab page. Upload screenshots that demonstrate your demo lab in action so that customers know what they can expect. You can also hover over images you have already uploaded to edit or remove them.

![preview images gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-preview-images.gif)

### User Manual

Here you can upload a PDF file or provide a web link to the user manual that will go along with the demo lab. Customers will need these to follow through with the scenarios you have created in the demo lab. 

![user manual gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-user-manual.gif)

### Supporting Documents

Here you can upload or provide a link to any other supporting documents to go along with your demo lab, including white papers, data sheets, or any other document.

![supporting documents gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-supporting-docs.gif)

###  Videos

Here you can upload a file (.mp4) or provide an embed link to any videos that you want to be available on the demo lab page showcasing the functionality of your demo lab.

![videos gif](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-videos.gif)

When you are done, click on the **Advanced Details** tab to continue.

## Advanced Details

### Demo Lab Duration

Note the approximate length (in minutes) that it would take a first-timer to complete the demo lab. This lets the customer know how much of a commitment they have to make to try out your solution.

![demo lab duration](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_demo-lab-duration.gif)

### Demo Lab Deploy Time

Note the approximate length (in minutes) that it would take for the demo lab to deploy. This lets the customer know how much lead time will be needed before they can try the demo lab.

![deployment time](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_demo-lab-deployment-time.gif)

### Concurrency Limit

Note the number of demo labs that can be active at the same time.

![concurrency limit](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_demo-lab-concurrency-limit.gif)

### Demo Labs per User Limit

Note the number of demo labs each unique user is allowed to deploy.

![user limit](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_demo-lab-user-limit.gif)

### Deployment Failure Message

Enter the message that will appear when a user's demo lab deployment fails for whatever reason. This message will be viewed by the customer.

![failure message](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_failure-message.gif)

### Concurrency Limit Message

Enter the message that will appear when a user has reached their concurrency limit. This message will be viewed by the customer.

![concurrency message](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_concurrency-message.gif)

### User Demo Lab Limit Message

Enter the message that will appear when a user reaches their limit of total demo labs deployments. This message will be viewed by the customer.

![user limit message](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_user-limit-message.gif)

### Outputs

These fields can customize and control how users view the demo lab output parameters. List the 
title of the output, followed by the output parameter value (e.g. This is the password: {password}).

![outputs](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-ad_outputs.gif)

Click the **Link Cloud Account** tab to continue.

## Link Cloud Account

### Link New Account

You will see tiles representing all of your registered cloud services accounts. Click on the one that you want to use to host the demo lab deployments. A green check mark will appear next to the account you have selected.

If you don't have a registered account or wish to add a new one, click the tile labeled **Link New Account**. This will open the fields necessary to link a new cloud account.

![link new account](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_link-new-account.gif)

### Account Name

Enter the name of your cloud account here.

![account name](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_account-name.gif)

### Account Description

Give your account a description to help keep track of which cloud account it is or include any important details about the account.

![account description](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_account-description.gif)

### Tenancy OCID

Enter the tenant OCID of the cloud account.

![tenancy ocid](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_tenancy-ocid.gif)

### User OCID

Enter the user OCID of the cloud account.

![user ocid](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_user-ocid.gif)

### Compartment OCID

Enter the compartment OCID of the cloud account.

![compartment ocid](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_compartment-ocid.gif)

### Fingerprint

Enter the cloud account "fingerprint" here.

![fingerprint](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_fingerprint.gif)

### Private Key Path

Enter the file path that leads to your account's private key here.

![private key](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-lca_private-key.gif)

When you are done, click **Save** to add this cloud account to your list of available accounts. Once the account is selected, click the **Project Details** tab to continue.

## Project Details

### Select Existing Project

If you have already created a project that you want to use to manage this demo lab, you can select it from the **Select an existing project** dropdown near the top of the section.

![select project](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_select-project.gif)

Otherwise, fill out the fields below to create a new project.

### Project Name

Give the project a unique name to stand out from your other projects.

![project name](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-name.gif)

### Project Description

Provide a description for the project and what it does.

![project description](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-description.gif)

### Fetch Project Details from:

Select either a GitHub account or an uploaded .zip file with which you will pull and maintain your project details.

#### GitHub Account

Clicking the GitHub account button creates the GitHub account forms.

![github project](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-details-from-github.gif)

**GitHub Account Name**

Enter the name of the GitHub account you will pull and manage your details from.

![github account name](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-details-github-account-name.gif)

**Select Repo**

Select the specific repository within the listed GitHub account.

![github select repo](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-details-github-select-repo.gif)

**Select Branch**

Select which branch of the listed repository you will pull and manage your details from.

![github select branch](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-details-github-select-branch.gif)

**Select Subfolder**

If applicable, select the subfolder to use within the listed branch.

![github select subfolder](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-details-github-select-subfolder.gif)

#### Upload Project Zip File

Clicking this option creates the **Upload Zip File** button, which prompts you to upload the zip file in question.

![upload zip](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-pd_project-details-upload-zip.gif)

When you are done, click the **Sharing Settings** tab to continue. 

## Sharing Settings

### Everyone

Check this option if you want anyone to be able to view or share this demo lab via social media or link sharing.

![everyone share](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-sharing_everyone.gif)

### Collaborators

Alternatively, you can set specific collaborators that can see the demo lab, organized by User, Team, Organization, or domain name (email address). Enter the individual emaill address, or the email domain, and select **Add** to add users.

![collaborators share](https://github.com/sysgain/qloudable-help/blob/master/images/demo-lab-sharing_collaborators.gif)

When everything is done, click **Publish** to create and publish the demo lab.
